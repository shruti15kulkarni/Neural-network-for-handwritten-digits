...
"""def preprocess():
    ...
    # Getting rid of columns in the image vector, which have same values throughout all the vectors
    to_delete = []
    for i in range(784):
        if np.ptp(train_data[:,i]) == 0.0 and np.ptp(validation_data[:,i]) == 0.0:
                to_delete.append(i)

    train_data = np.delete(train_data, to_delete, axis=1)
    validation_data = np.delete(validation_data, to_delete, axis=1)
    test_data = np.delete(test_data, to_delete, axis=1)
    end = time.clock()
...
def preprocess():
    ...
    indices=np.arange(0,785)
    global selected_features
    selected_features=list(set(indices)-set(delete_indexes))
    #print final_indices
...
def nnObjFunction(params, *args):
    ...
    aj0 = np.dot(training_data,w1.T)
    zj0 = sigmoid(aj0)

    # Feed Forward from Hidden to Output
    training_data_2 = np.concatenate((zj0,np.ones([zj0.shape[0],1])),axis =1)
    bl = np.dot(training_data_2,w2.T)
    ol = sigmoid(bl)
    ...
    for i in range(0,training_label.shape[0]):
        #temp=training_label[i].astype(int)
        Y[i,training_label[i].astype(int)] = 1
        
    #num_frames = training_label.shape[0]
    #Y = np.zeros((num_frames, 10))
    #Y[np.arange(num_frames), training_label[0,:]] = 1

    # Back Propogation (Calculating Delta and Error)

    delta = np.multiply(Y - ol, (1 - ol) * ol)
    error = (np.sum(np.square(Y-ol)))/2

    # Calculating gradient of w1 and w2
    # Derivates of w1 and w2 along with Regularization
    grad_w1 = -(np.dot(training_data.T,(np.dot(delta,w2) * (training_data_2 * (1 - training_data_2))) ))
    grad_w1 = grad_w1[:,:-1].T
    grad_w1 = (grad_w1+(lambdaval * w1))/(training_data.shape[0])
    
    grad_w2 = -(np.dot(delta.T,training_data_2))
    grad_w2 = (grad_w2+(lambdaval * w2))/(training_data.shape[0])

    obj_val = (error/(training_data.shape[0]))  +  ((lambdaval/(2 * training_data.shape[0])) * (np.sum(np.square(w1)) + np.sum(np.square(w2))))
...
def nnPredict(w1, w2, data):
    ...
    training_data = np.concatenate((data,np.ones([data.shape[0],1])),axis = 1)
    aj0 = np.dot(training_data,w1.T)
    zj0 = sigmoid(aj0)
...
print('\n Number of hidden nodes: ',n_hidden)
#print('\n number of iter', )
...
"""end_full=time.clock();
print('full running time',end_full-start_full)
objects = []
with (open("params.pickle", "rb")) as openfile:
    while True:
        try:
